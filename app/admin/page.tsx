...
const ITEMS_PER_PAGE = 10; // Cantidad de camisetas por página en la tabla admin
...
export default function AdminPage() {
   ...
   // 2. Cargar datos relacionales desde Supabase
   const fetchAdminData = async () => {
      ...
      try {
         ...
         if (tallasData) {
            setAllTallas(tallasData as any);
         }
         ...
         const { data: productosData, error } = await supabase
            ...
            .select(`
               ...
               productos_tallas (
                  talla_id,
                  tallas ( id, nombre, orden, grupo ( id, nombre ) )
               )
            `)
         ...
         if (productosData) {
            const mappedProducts: Product[] = productosData.map((p: any) => {
               ...
               const lineId = firstLine ? String(firstLine.id) : "";
   ...
   useEffect(() => {
      setCurrentPage(1);
   }, [search, lineFilter, sizeFilter, statusFilter]);

   // CÁLCULO DE PAGINACIÓN ADMIN
   ...
   function openNew() {
      ...
      setForm({
         ...EMPTY_FORM,
         lineId: initialLine,
         sizes: allTallas.map((t) => t.nombre),
         unavailableSizes: [],
      });
      setEditorOpen(true);
   }
   ...
   function changeLine(lineId: string) {
      setForm((current) => ({
         ...current,
         lineId,
      }));
   ...
   function toggleSize(sizeName: string) {
      setForm((current) => {
         ...
         return {
            ...current,
            sizes: exists
               ? current.sizes.filter((s) => s !== sizeName)
   ...
   // ⚡ TOGGLE DE AGOTADO INDIVIDUAL CON SERVER ACTION
   async function toggleSoldOut(product: Product) {
      ...
      if (res.success) {
         setProducts((prev) =>
            prev.map((item) =>
               item.id === product.id ? { ...item, soldOut: nextStatus } : item
            )
         );
   ...
   // ⚡ CAMBIO MASIVO DE ESTADO (ACCION BULK) CON SERVER ACTION
   async function handleBulkSoldOut(soldOut: boolean) {
      ...
      if (res.success) {
         setProducts((prev) =>
            prev.map((product) =>
               selectedIds.includes(product.id) ? { ...product, soldOut } : product
            )
         );
         setSelectedIds([]);
   ...
   function toggleSelectAll() {
      if (allVisibleSelected) {
         setSelectedIds((current) =>
            current.filter((id) => !paginatedProducts.some((product) => product.id === id))
         );
         return;
      }

      setSelectedIds((current) => [
         ...new Set([...current, ...paginatedProducts.map((product) => product.id)]),
      ]);
   }
   ...
   return (
      <main className="admin-page">
         ...
         <section className="admin-content">
            ...
            <div className="admin-main">
               ...
               <div className="admin-toolbar">
                  ...
                  {/* ACCIONES MASIVAS CUANDO HAY PRODUCTOS SELECCIONADOS */}
                        ...
                        <article className="admin-table-row" key={product.id}>
                           ...
                           <div>
                              <img
                                 ...
                                 onClick={() =>
                                    setLightboxImage({ url: product.image, title: product.name })
                                 }
            ...
            >
               <form className="admin-editor-modal" onSubmit={saveProduct}>
                     ...
                     <div className="admin-upload-field full">
                        ...
                        >
                           {form.image ? (
                              ...
                              <>
                                 <div className="admin-upload-icon"><FaUpload /></div>
                  ...
                  {Object.keys(groupedTallas).length > 0 ? (
                              ...
                              {sizesGroup.map((talla) => {
                                 ...
                                 return (
                                    <button
                                       type="button"
                                       key={talla.id}
                                       className={`admin-size-btn ${!isAvailable ? "off" : ""}`}
                                       onClick={() => toggleSize(talla.nombre)}
                                    >
                                       <strong>{talla.nombre}</strong>
                                       <span>{isAvailable ? "Disponible" : "Agotada"}</span>
                                    </button>
                                 );
                              })}
                           </div>
                     ...
                     <div className="admin-size-grid">
                        <span style={{ fontSize: "0.85rem", color: "#9ca3af" }}>Cargando tallas desde la base de datos...</span>
                     </div>
                  )}
                  ...
                  <div className="admin-modal-actions">
                     <button type="button" className="admin-cancel-btn" onClick={closeEditor} disabled={saving}>
                        Cancelar
                     </button>
                     <button type="submit" className="admin-save-btn" disabled={saving}>
                        {saving ? "Guardando..." : "Guardar camiseta"}
                     </button>
                  </div>
               </form>
            </div>
         ...
         {/* MODAL LIGHTBOX / VISTA AMPLIADA DE LA IMAGEN */}
         {lightboxImage && (
            <div
               className="admin-modal-backdrop"
               style={{
